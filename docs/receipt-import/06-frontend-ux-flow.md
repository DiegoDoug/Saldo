...
- **Inline merchant/category creation.** Today, the transaction form's merchant/category are plain `<select>`s with no "create new" affordance (Document 1 §3) — a receipt scan of an unrecognized merchant needs one. `EntityCombobox.tsx` generalizes `TagInput`'s create-on-type pattern (already in the codebase for tags) from a multi-select of free text to a single foreign-key selection: typing a name that doesn't match shows a "Crear «name»" option, selecting it creates the row immediately (not deferred to confirm) so the new id is available like any other option for the rest of the review. **As built (Stage 5), this calls the local Dexie `localRepo` functions directly** (`merchants/localRepo.ts`'s `addMerchant`, `budgeting/localRepo.ts`'s `addCategory`) rather than this section's original `POST /merchants`/`POST /budgeting/categories` sketch — simpler and more consistent with the rest of the app's offline-first discipline: the new row syncs up on the next push exactly like the transaction itself, with no direct-POST-plus-Dexie-mirror duplication to keep in sync.
...
- Feature disabled (`503`): originally planned as hiding the button entirely via a boot-time capabilities check (see below). **As built (Stage 4):** no such endpoint existed to piggyback on, and adding one just for this one flag would be premature — the button always renders (gated only on having an account and being online) and the dialog surfaces the backend's 503 inline ("El escaneo de recibos no está configurado en este servidor.") on the first upload attempt, then returns to the capture step. Functionally equivalent for a self-hoster who hasn't set a key: one clear message instead of a hidden button. Revisit the original plan below if a real settings/capabilities endpoint appears for other reasons: checked once via a small capabilities flag alongside the app-boot config fetch, not a new endpoint — Document 5 §2's `deepseek_enabled` could ride on the same settings surface the frontend reads at boot, e.g. `GET /health` or a comparable existing check.
